...
import trends from '../../assets/images/trends_shaping.png'
import b_2025 from '../../assets/images/Blog_2025.jpg'
import b_christmas from '../../assets/images/Blog_Christmas.jpg'
import b_spenmax from '../../assets/images/Blog_spenmax.jpg'
import b_initicure from '../../assets/images/Blog_initicure.jpg'
import { picture } from 'framer-motion/client'
import stroke from '../../assets/images/colourfull.png'
...
function Blogs() {
    ...
    const blogs = [
        ...
        {
            "title": 'Welcome to the era of the art of development , Where Vision meets code',
            'body': [
                {
                    'type': 'p',
                    'content': 'A fleeting examination of the big trends and whats in store for digital transformation in 2021.'
                },
                {
                    'type': 'h1',
                    'content': 'Redefining digital transformation'
                },
                {
                    'type': 'p',
                    'content': 'Companies dealing with digital transformation used to be technologically specific. But today, digital innovations determine the future for corporations across industries. These changes will affect the function of organizations across all sectors over the coming year.'
                },
                {
                    'type': 'h2',
                    'content': '1. Smarter AI (Artificial intelligence)'
                },
                {
                    'type': 'p',
                    'content': 'Everywhere from e-commerce to cybersecurity, education, healthcare, is Artificial Intelligence (AI) essential. Today, AI controls tools such as chatbots, virtual assistants, and even autonomous vehicles. In the future, the emphasis will be on enhancing AI for improved understanding of individual responses and reactions – an essential advantage for companies that wish to communicate with customers digitally.'
                },
                {
                    'type': 'p',
                    'content': 'The COVID-19 pandemic has placed more pressure on corporations to continue to transform digitally with everything moving from offline to online. The economic recession has driven businesses to accept remote workforce structures and change recruiting models. Digital innovation activities are accelerating pandemic-inspired challenges, and global digital technology and services investment is projected to rise by 10.4% for the year 2021.'
                }
            ],
            'picture': b_2025,
            'date': '11/12/2024',
        ...
        {
            'title': 'This festival season Appmine studio wraps up the year with gratitude and exitement ',
            'picture': b_christmas,
            'date': '11/12/2024',
            'bywhom': 'Joel Baiju',
            'profile': ''
        ...
        {
            'title': 'Discover exclusive deals from local store with spenmax',
            'picture': b_spenmax,
            'date': '11/12/2024',
            'bywhom': 'Joel Baiju',
            'profile': ''
        ...
        {
            'title': 'We are proud to share the success story of our collaboration with Inticure',
            'picture': b_initicure,
            'date': '11/12/2024',
            'bywhom': 'Joel Baiju',
            'profile': ''
    ...
    return (
        <div className=' h-screen bg-white md:p-10'>
            <div className='w-full h-25 p-10 flex  items-end'>
                <h1 className='text-neutral-800 text-4xl astralaga'>Blogs</h1>
            </div>
            <svg className='h-0'>
                <defs>
                    <clipPath id="brushclip" clipPathUnits="objectBoundingBox">
                        <use href="#brush-shape" className='' clip-path="url(#rect-cp-indefinite)" />
                    </clipPath>
            ...
            <div className='flex flex-col md:flex-row md:justify-between w-screen h-full  md:py-10 gap-5'>



                <div className='w-full md:w-5/12 md:h-3/5 h-2/6 relative gmarket'>
                    <img src={stroke} alt="My Image" className=' h-full clipped-image z-10 w-full absolute blur-xl' />
                    <div className='absolute w-full flex-col flex justify-center items-center h-full z-20'>
                        <h1 className='text-neutral-900 text-lg astralaga'>Inspired by Art, Powered by Code</h1>
                        <p className='text-neutral-800 w-8/12'>Inspired by the elegance of art and powered by the precision of code, we craft solutions that not only meet your needs but exceed your expectations.</p>
                    </div>
                </div>
                ...
                {/* 
                <div  className=' overflow-x-scroll w-3/5 flex flex-row remove_scrollbar border'>
                    {blogs.map((blog, index) => (
                        <div className='w-100 h-100 relative rounded-3xl gmarket shadow-2xl black-shadow border-[1px] border-neutral-500 '>
                            <img src={blog['picture']} className='absolute rounded-3xl z-10 rounded-t-3xl ' alt="" />
                            <div className='w-full h-full flex items-end rounded-3xl'>
                                <div className='p-5 h-1/2 backdrop-blur-md bg-black/55 rounded-b-3xl absolute z-20'>
                                    <h1 className='text-sm text-white font-semibold'>{blog.title}</h1>
                                    <div className='flex justify-between items-end'>
                                        <div className='flex items-end gap-2'>
                                            <div className='p- rounded-full bg-neutral-700 h-7 w-7 flex items-center justify-center'> <i className="fa-solid fa-user text-white"></i></div>
                                            <p className='text-white text-sm  mt-5'>{blog.bywhom}</p>
                                        </div>
                                        <p className='text-white text-xs mt-5'>{blog.date}</p>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="overflow-x-scroll p-3 w-full md:w-7/12 flex flex-row gap-4 remove_scrollbar ">
                    {blogs.map((blog, index) => (
                        <div
                            key={index}
                            className="min-w-[300px] h-[400px] relative rounded-3xl gmarket border-[1px] border-black flex-shrink-0"
                        >
                            <img
                                src={blog['picture']}
                                className="absolute top-0 left-0 w-full h-[60%] rounded-t-3xl object-cover z-10"
                                alt=""
                            />
                           <div className='w-full h-full flex items-end rounded-3xl'>
                                <div className='p-5 h-1/2 backdrop-blur-md bg-black/55 rounded-b-3xl absolute z-20'>
